package com.qubacy.geoqq.ui.common.visual.component.animatedlist.adapter

import android.util.Log
import androidx.recyclerview.widget.RecyclerView
import androidx.recyclerview.widget.RecyclerView.ViewHolder
import com.qubacy.geoqq.ui.common.visual.component.animatedlist.animator.AnimatedListItemAnimatorCallback
...
abstract class AnimatedListAdapter<ViewHolderType : ViewHolder, ItemType>(
    ...
    protected open fun processAddItem(item: ItemType): Int {
        if (mIsReversed)
            mItemAdapterInfoList.add(0,
                ItemAdapterInfo(
                    item
                )
            )
        else
            mItemAdapterInfoList.add(
                ItemAdapterInfo(
                    item
                )
            )
    ...
    override fun wasViewHolderAnimated(viewHolder: ViewHolder): Boolean {
        val messageAdapterInfo = mItemAdapterInfoList[viewHolder.adapterPosition]

        return messageAdapterInfo.wasAnimated
    }

    override fun setViewHolderAnimated(viewHolder: ViewHolder) {
        val messageAdapterInfo = mItemAdapterInfoList[viewHolder.adapterPosition]
